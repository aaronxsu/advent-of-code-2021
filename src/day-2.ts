...
const finalUnitMultipliedTwo = (input: string[]): number => {
  if (input.length === 0) return 0;
  let horizontal = 0;
  let vertical = 0;
  let aim = 0;
...
console.log(`Puzzle 1: ${finalUnitMultiplied(directions)}`);
console.log(`Puzzle 2: ${finalUnitMultipliedTwo(directions)}`);
